...
local eventTaunts = {'CursedFlashlight','WerewolfHowl','Tombstone','Baghead','DemonHorns','Candlehead','ImpaledHead','WitchHat','DualBoneSwords','LavaLamp','SpiderInfestation','PumpItUp','VampireOutfit','OminousDemise','BlueHallowedFace','GreenHallowedFace','OrangeHallowedFace','GhostCatcher','DemonWings','CandleLamp','FungalOvergrowth','Reanimated','ToxicInferno','Monoculi','WretchedWings','PhantomBlades','AmethystStaff','GhostAbduction','Telespell','Thriller','BluefirePortal','HellfirePortal','SeekingEye','EyeCorruption','Necromonicon','FrightFunk','BlueUFOAbduction','PinkUFOAbduction','Cauldronhead','HellishGrip','HauntedIredescence','CobwebTrap','MechanicalScythe','DragonSkull','GhostFriend','SpilledCauldron','WardingLantern','RockinStride',"Rockin'Stride",'DarkTendrils','AlchemistBelt','Frankenstein','ElectrifyingGuitar','BatVision','BananaSuit','EyeIllisions','BloodMoon'}
...
local main = lib:CreateTab({
    Name = "Main"
})

local mesc = lib:CreateTab({
    Name = "Misc"
})

local misc = mesc:CreateSection({
    Name = 'Misc',
    Side = 'Left'
})

local player = main:CreateSection({
    Name = "Player",
    Side = 'Left'
})
...
local map = main:CreateSection({
    Name = "Map",
    Side = 'Right'
})

local troll = main:CreateSection({
    Name = "Trolling",
    Side = 'Right'
})
...
player:AddToggle({
    ...
    Callback = function(x)
       ...
       while task.wait(1) and respawning do
        if not getgenv().respawning then break end
